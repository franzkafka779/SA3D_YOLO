...
# 환경 변수로 로깅 레벨 제어
import os
log_level = os.environ.get('DASH_LOG_LEVEL', 'WARNING').upper()

# Dash와 Flask 관련 로거의 레벨을 조정하여 반복 로그 줄이기
logging.getLogger('dash').setLevel(getattr(logging, log_level))
logging.getLogger('werkzeug').setLevel(getattr(logging, log_level))
logging.getLogger('dash.dependencies').setLevel(getattr(logging, log_level))
    ...
    def run_app(self, sam_pred, ctx, init_rgb):
        ...
        def query(points=None, text=None):
            with torch.no_grad():
                if text is None:
                    ...
                    # 하드코딩으로 idx_select 설정
                    # 데이터셋에 따라 적절한 값으로 수정하세요
                    # 예: Set5의 경우 0-4 범위, 다른 데이터셋은 0-8 범위 등
                    idx_select = 0  # 여기에 원하는 인덱스를 입력하세요
                    
                    # YOLO가 감지한 객체 수 확인 (디버깅용)
                    num_detected_objects = len(results[0].masks.data)
                    print(f"YOLO detected {num_detected_objects} objects, using index {idx_select}")
                    
                    # 선택한 인덱스가 유효한 범위인지 확인
                    if idx_select >= num_detected_objects:
                        print(f"Warning: idx_select {idx_select} is out of range (0-{num_detected_objects-1})")
                        # 범위를 벗어나면 마지막 인덱스 사용
                        idx_select = num_detected_objects - 1
                        print(f"Adjusted to {idx_select}")
                    
                    # 현재 뷰 인덱스를 idx_selected에 저장
                    if hasattr(self.Seg3d, 'idx_selected') and isinstance(self.Seg3d.idx_selected, dict):
                        # 현재 뷰 인덱스를 찾기 위해 data_dict에서 확인
                        if hasattr(self.Seg3d, 'data_dict') and 'i_train' in self.Seg3d.data_dict:
                            current_view_idx = self.Seg3d.data_dict['i_train'][0] if len(self.Seg3d.data_dict['i_train']) > 0 else 0
                            self.Seg3d.idx_selected[current_view_idx] = idx_select
        ...
        def start_training(btn):
            if btn < 1:
                ...
                # 디버깅 정보 출력
                print(f"Debug - confidences length: {len(confidences)}")
                print(f"Debug - i_train length: {len(i_train)}")
                print(f"Debug - idx_selected keys: {list(self.Seg3d.idx_selected.keys()) if hasattr(self.Seg3d, 'idx_selected') else 'No idx_selected attribute'}")

                # 데이터 유효성 검사 추가
                if len(confidences) == 0 or len(i_train) == 0:
                    return html.Div("Error: No training data available. Please check your dataset."), no_update, no_update
                ...
                # i_train_sorted가 비어있는지 확인
                if len(i_train_sorted) == 0:
                    return html.Div("Error: No sorted training indices available. Please check your dataset."), no_update, no_update
                ...
                # 첫 번째 인덱스가 idx_selected에 존재하는지 확인
                first_idx = i_train_sorted[0]
                if first_idx not in self.Seg3d.idx_selected:
                    return html.Div(f"Error: Index {first_idx} not found in idx_selected. Available indices: {list(self.Seg3d.idx_selected.keys())}"), no_update, no_update
                ...
                # YOLO가 객체를 인식하지 못했는지 확인
                if not hasattr(results[0], 'masks') or results[0].masks is None or len(results[0].masks.data) == 0:
                    return html.Div(f"Error: YOLO detected no objects in image {first_idx}. Please check if the image contains detectable objects."), no_update, no_update
                
                h2, w2 = results[0].masks.data[0].shape
                m = torch.zeros([h, w])
                
                # 하드코딩으로 idx_select 설정 (첫 번째 위치와 동일하게)
                # 데이터셋에 따라 적절한 값으로 수정하세요
                idx_select = 0  # 여기에 원하는 인덱스를 입력하세요
                
                # YOLO가 감지한 객체 수 확인 (디버깅용)
                num_detected_objects = len(results[0].masks.data)
                print(f"Training view {first_idx}: YOLO detected {num_detected_objects} objects, using index {idx_select}")
                ...
                # idx_selected에 저장 (나중에 사용할 수 있도록)
                if hasattr(self.Seg3d, 'idx_selected') and isinstance(self.Seg3d.idx_selected, dict):
                    self.Seg3d.idx_selected[first_idx] = idx_select
                
                # idx_select가 유효한 범위인지 확인
                if idx_select >= len(results[0].masks.data):
                    return html.Div(f"Error: Invalid mask index {idx_select}. Available mask indices: 0-{len(results[0].masks.data)-1}. This usually means YOLO detected fewer objects than expected."), no_update, no_update
