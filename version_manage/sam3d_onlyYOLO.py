    ...
    def train_step(self, idx, sam_mask=None):
        ...
        if sam_mask is None:
            ...
            if not hasattr(results[0], 'masks') or results[0].masks is None or len(results[0].masks.data) == 0:
                print(f"Warning: YOLO detected no objects in image {idx}. Skipping this step.")
                return None, None, True  # 학습 종료 신호
            ...
            # iou 리스트가 비어있는지 확인
            if len(iou) == 0:
                print(f"Warning: No valid IoU values calculated for image {idx}. Skipping this step.")
                return None, None, True  # 학습 종료 신호
            ...
            if self.vsgflag == False:
                ...
                self.idx_selected[idx] = iou.index(max(iou))
